# 職務経歴書(2020.09.10現在)

## スキル

### プログラミング言語

|プログラミング言語|バージョン|実務経験|実務以外での利用|
|-----|-----|-----|-----|
|PHP|5.0~7.2|1年||
|SQL||4年||
|JavaScript|ECMAScript 2015|5ヶ月||
|HTML5||5ヶ月||
|SCSS||5ヶ月||
|VB.NET||1年||
|COBOL||1年未満||
|Ruby|2.6||3ヶ月|

### フレームワーク

|フレームワーク|バージョン|実務経験|実務以外での利用|
|-----|-----|-----|-----|
|Laravel|6.0|5ヶ月||
|Vue.js|2.5|5ヶ月||
|Vuetify|2.2|5ヶ月||
|ASP.NET||6ヶ月||
|Ruby on Rails|5.2||3ヶ月|

### DB

|DB|バージョン|実務経験|実務以外での利用|
|-----|-----|-----|-----|
|MySQL|8.0|5ヶ月||
|PostgreSQL||3ヶ月||
|Microsoft SQL Server|2015~2017|1.5年||
|Teradata DB||2年||

### バージョン管理ツール

|ツール|実務経験|実務以外での利用|
|-----|-----|-----|
|Git|9ヶ月||
|GitHub|5ヶ月||
|SVN|6ヶ月||

### その他

|ツール|実務経験|実務以外での利用|
|-----|-----|-----|
|Docker|5ヶ月||
|AWS|1ヶ月||
|Slack|8ヶ月||
|Zoom|8ヶ月||

## 言語

- 日本語
  - ネイティブ
- 英語
  - 簡単な読み書きができる

## 職務経歴

### 2020/04 ~ : HR Tech企業

#### SPAの開発
- Dockerで開発環境の構築
- S3ファイルアップロード機能の作成(Minio)
- S3ファイルダウンロード機能の作成(Minio)
- MDIを使用したUIデザインの修正


### 2014/9 ~ 2019/9 : SES系企業

### 1. 軽減税率開始に伴うシステム改修
**小売 / マーチャンダイジングシステム**
#### ◆期間
- 2019年03月 ~ 2019年08月
#### ◆担当業務
- バッチ処理の改修
- 初期データ作成プログラムの作成 
#### ◆担当フェーズ
- 製造 ~ 結合テスト
#### ◆環境
- Windows Server
#### ◆言語/DB
- Transact-SQL
- VB.NET
- Microsoft SQL Server 
#### ◆マシン
- Windows10
##### ◆その他ツール
- SVN
#### ◆チーム人数 / 役割
- 7名 / メンバー

### 2. システムのPHPバージョンアップに伴う改 修及び環境移行作業
**エンターテイメント企業 / 基幹システム**
#### ◆期間
- 2018年11月 ~ 2019年02月
#### ◆担当業務
- PHPバージョンアップに伴う エラー原因の調査とプログラムの改修
- 環境移行のためのApacheの設定 
- 環境移行のためのSQL Serverの設定
#### ◆担当フェーズ
- 切り分けは特になし
#### ◆環境
- Windows Server
- Apache
#### ◆言語/DB
- PHP
- Smarty
- Microsoft SQL Server
#### ◆マシン
- Windows10
#### ◆その他ツール
- Git
#### ◆チーム人数 / 役割
- 4名 / メンバー

### 3. 企業統合に伴うシステム改修
**小売 / マーチャンダイジングシステム**
#### ◆期間
- 2018年08月 ~ 2018年10月
#### ◆担当業務
- バッチ処理の改修
- UI画面の改修 
#### ◆担当フェーズ
- 製造 ~ 単体テスト
#### ◆環境
- Linux
- Apache
#### ◆言語/DB
- Bash
- PHP
- PostgreSQL
#### ◆マシン
- Windows10
#### ◆その他ツール
- Slack
#### ◆チーム人数 / 役割
- 6名 / メンバー

### 4. 製造実行システムパッケージ製品のカス タマイズ
**電気系メーカー / 製造実行システム**
#### ◆期間
- 2017年10月 ~ 2018年07月
#### ◆担当業務
- 在庫管理機能改修 
- マスタデータ管理機能 
- ハンディターミナル入力機能改修
#### ◆担当フェーズ
- 製造 ~ 結合テスト
#### ◆言語/DB
- VB.NET
- ASP.NET
- Microsoft SQL Server
#### ◆マシン
- Windows7
#### ◆チーム人数 / 役割
- 11名 / メンバー

### 5. システム保守
**地方銀行 / 情報系システム**
#### ◆期間
- 2015年12月 ~ 2017年09月
#### ◆担当業務
- バッチ処理の改修 
- 他システムデータ連携機能の開発 
- 各種EXCEL帳票の開発
#### ◆担当フェーズ
- 保守要員
- 詳細設計 ~ リリース作業
#### ◆環境
- Systemwalker 
- AccessNavigator Web 
#### ◆言語/DB
- Teradata-SQL
- VBA
- VBS
#### ◆マシン
- Windows7
#### ◆チーム人数 / 役割
- 11名 / メンバー

### 6. 外国為替関連システム刷新に伴うシステ ム改修
**都市銀行 / 外国為替関連システム**
#### ◆期間
- 2015年01月 ~ 2015年11月
#### ◆担当業務
- ファイル入出力機能の開発 
#### ◆担当フェーズ
- 詳細設計 ~ 結合テスト
#### ◆環境
- OSIV/F4 MSP
#### ◆言語/DB
- COBOL
- JCL
#### ◆マシン
- Windows Vista
#### ◆チーム人数 / 役割
- 8名 / メンバー

### 7. 新人研修
#### ◆期間
- 2014年09月 ~ 2014年12月
#### ◆担当業務
- Access, VBA, SQLを中心に研修を行う
#### ◆言語/DB
- VBA
- Microsoft SQL Server
#### ◆マシン
- Windows 7

### 業務での実績
- EXCELドキュメント内のgrep検索ツールの作成
- PHP5.3からPHP7.2へのバージョンアップに伴って発生するエラー原因の調査と改善策の提案
- 他者設計のロジックの問題点の指摘と改善策の提案

## Social

|key|value|
|---|-----|
|Qiita|[@3no3](https://qiita.com/3no3_tw)|
|Twitter|[@3no3](https://twitter.com/3no3_tw)|
|GitHub|[rikuro-aub](https://github.com/rikuro-aub)|

## その他特記事項
- [Vue.jsのフレームワークを使用して自己紹介サイトを作成](https://rikuro-aub-portfolio.netlify.com)
- [Ruby on Railsのフレームワークを使用してプログラミング学習動画に特化した動画掲載&共有サービスを作成](https://proitt.herokuapp.com/)
- プライベートではMacを使用して開発を行っています
